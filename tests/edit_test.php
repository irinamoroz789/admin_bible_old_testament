                    ...
                    while ($question = $result_2->fetch_assoc()) {
                        ...
                        <div id="inputFormQuestion_<?= $i ?>" class="inputFormQuestion" data-answer_count="1">
                            <div class="delete-question>">
                                <button id="removeQuestion" class="button-trash" type="button"
                            ...
                            if ($i == $number_question - 1) {
                                ?>
                                <div id="addQuestion" onclick="addQuestion(this)"><span class="noselect"
                                                                                        onchange="addAnswer()">Добавить вопрос</span>
                                    <div id="circle"></div>
                                </div><br>
                                <?php
                            }
                            ?>
                            <input type="text" class="question" name="question_<?= $i ?>" placeholder="Текст вопроса"
                            ...
                            foreach ($answers as $answer) {
                                ...
                                <div id="inputFormAnswer_<?= $i ?>_<?= $j ?>" class="inputFormAnswer">
                                    <div class="input-group">
                                        <input type="text" name="answer_<?= $i ?>_<?= $j ?>"
                                               class="form-control m-input" placeholder="Введите ответ"
                                               autocomplete="off" value='<?= $answer ?>'>
                                        <div class="input-group-append">
                                            <button id="removeAnswer" type="button" class="btn-danger"
                            ...
                            ?>
                            <input type="hidden" name="answer_count[]" id="input_answer_count_<?= $i ?>"
                                   class="answer_count" value="<?= $j - 1 ?>">
                            <div class="newAnswer"></div>
                            <div id="addAnswer" class="add_answer" onclick="addAnswer(this)">+</div>
                            <br>
                            <input type="number" name="response_answer_<?= $i ?>" class="response_answer" min="1"
                                   step="1" placeholder="Номер ответа" value='<?= $question['answer'] ?>'
                                   onchange="this.value = parseInt(this.value);"><br>
                            <div class="input-file-row">
                                <label class="input-file">
                                    <input type="file" name="image_<?= $i ?>" class="image" multiple accept="image/*"
                                           onchange="changeImage(this)"><br>
                                ...
                                <div class="input-file-list">
                                    ...
                                    if (@getimagesize($question['image'])) {
                                        ...
                                        <div class="input-file-list-item" id="input_image<?= $i ?>">
                                            ...
                                            <input type="hidden" name="save_image_<?= $i ?>" id="save_image_<?= $i ?>"
                                                   class="save_image" value="<?= $question['image'] ?>">
                                        </div>
                            ...
                            <br>
                            <input type="text" name="img_caption_<?= $i ?>" class="img_caption"
                                   placeholder="Описание картинки" value='<?= $question['img_caption'] ?>'><br>
                            <textarea placeholder="Комментарий к ответу..." name="comment_<?= $i ?>" class="comment"
                                      rows="3"><?= $question['comment'] ?></textarea><br>
